...
单次扫描直接读取微信主窗口中的目标群，不要求提前打开独立聊天窗口。已在目标群时不会重复切换；否则只尝试一次精确切换。读取前后都会检查群名，切到其他聊天会停止本次扫描。看到 `[POLL_ONCE_DONE]` 只表示读取完成；随后出现“已发送到 Worker”才表示收到提取结果。`scan-once.cmd` 已启用 UTF-8，避免中文提示乱码。
